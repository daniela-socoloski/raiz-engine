# 15 — R2 marca-raiz Routine (Routine Local)

> **Não é página do Notion.** É a configuração da **Routine Local** que roda dentro do Claude Code Desktop app. O R2 do marca-raiz é o **agente operacional** — gera, audita e evolui aplicação do marca-raiz. Roda sob demanda (não no schedule diário do Carrossel — diferente).
>
> Se o R2 do Carrossel é "produz UM carrossel/dia automaticamente", o R2 do marca-raiz é "responde a brief, audita asset, ou conduz mini-discovery sob demanda".
...
## O que o R2 do marca-raiz faz — os 4 modos

| Modo | Quando ativa | O que entrega |
|---|---|---|
| **`generate`** | Brief de criação (cole texto + tipo de peça) | Output gerado seguindo marca-raiz — copy + imagem + arquivos |
...
| **`evolve`** | Mini-discovery de evolução (8 perguntas) | Diff proposto pro marca-raiz + aplicação após aprovação |
...
- Working folder local `~/{brand-slug}/` com `docs/`, `.brand.json`, `.marca-raiz.json`, `notion-ids.json`
...
```
~/{brand-slug}/
├── docs/                       # 18 arquivos .md do marca-raiz
├── .claude/
│   └── settings.json           # allowlist de permissões
├── .brand.json                 # variáveis canônicas
├── .marca-raiz.json                   # snapshot completo do marca-raiz (cache)
...
**Nenhuma.** Toda config vem de `.brand.json`, `.marca-raiz.json`, `notion-ids.json` e da página `🔐 Configuração` do Notion.

### Schedule
- Tipo: **Manual** (não cron)

> R2 do marca-raiz é **sob demanda** — você ativa quando precisa, não automaticamente. Diferente do R2 do Carrossel.
...
````
Você é o R2 — marca-raiz Routine da {brand_name}. Sua função é operar o marca-raiz da marca em 
...
3. Carregue ./notion-ids.json e ./.brand.json e ./.marca-raiz.json.
...
5. IDENTIFIQUE O MODO da primeira mensagem:
   ...
   - "evolve" ou "evoluir marca-raiz" → MODO EVOLVE
   - vazio ou "?" ou "ajuda" → MODO BRIEF (lista jobs pendentes)

6. CARREGA O marca-raiz: leia .marca-raiz.json (snapshot rápido). Para profundidade, leia páginas 
...
3. AUDITORIA — pontue 4 dimensões (0-10 cada):
   ...
   Compare contra docs/06-Audience-marca-raiz.md + page_audience. Critério:
...
5. SUGESTÃO DE REESCRITA (se aplicável):
   Se asset é texto, ofereça versão reescrita aderente ao marca-raiz.
...
4. PROCESSA O BRIEF — enriquecimento:
   - Lê o marca-raiz relevante (.marca-raiz.json + páginas Notion específicas)
   - Aplica voz da marca (07-Voice-and-Tone)
   - Filtra contra anti-patterns (12-Anti-Patterns)
   - Verifica se cabe nos pilares (marca-raiz)
...
5. GERA o output conforme tipo de peça:
   ...
   - Cada seção respeita marca-raiz
...
6. AUTO-AUDIT FINAL: roda Dimensões A e B do MODO AUDIT no output completo. 
   ...
   onde está abaixo do marca-raiz.
...
============================================================
MODO 3 — EVOLVE (mini-discovery de evolução do marca-raiz)
============================================================

Você recebeu: "evolve" ou "evoluir marca-raiz" ou "atualizar marca-raiz"
...
3. APRESENTA SÍNTESE em formato diff:
   ...
   PROPOSTA DE EVOLUÇÃO DO marca-raiz
...
4. Após aprovação:
   ...
   - Atualiza .brand.json e .marca-raiz.json
   - Append em "Histórico de mudanças" da página 🧬 marca-raiz
   - Cria snapshot do marca-raiz pré-mudança no Drive (marca-raiz-Snapshots/{TODAY}-pre-evolve/)
...
6. OUTPUT:
   ...
   Histórico atualizado em 🧬 marca-raiz

   Próximas runs de QUALQUER agente (R2 marca-raiz, R2 Carrossel, futuros) já leem o marca-raiz novo.
...
3. Consulta page_pending_evolutions (sub-página em marca-raiz se existir):
   - Lista evoluções sugeridas (R1 pode propor, ex: análise de tendência sugere refresh)

4. OUTPUT:

   R2 marca-raiz — STATUS DO DIA
...
- marca-raiz é fonte da verdade. Sempre lê antes de gerar/auditar.
...
- Nenhuma mudança no marca-raiz sem MODO EVOLVE explícito.
...
> generate: <novo brief>
> mostra meu marca-raiz
> qual nosso anti-pattern visual #2?
> rebuild .marca-raiz.json
...
R2 do marca-raiz é **stateless por job**. Cada Run now é independente. Não tem `--re-render` (porque cada job é único).
...
- Página `🧬 marca-raiz` → "Histórico de mudanças" atualizado após evolve
...
R2 do Carrossel **lê o marca-raiz antes de qualquer execução**. Após o setup do marca-raiz-prisma:

1. Atualize o prompt da Routine R2 do Carrossel
2. Adicione **antes da Etapa 0**:

```
ETAPA -1 — Carregar marca-raiz-prisma
- Lê page_dna_master via MCP Notion
- Carrega .marca-raiz.json local
- Variáveis brand_* vêm DAQUI (não da página 🏷️ Brand Identity legacy)
- Manual editorial passa por filtro do 🗣️ Voice & Tone do marca-raiz
- Visual System do marca-raiz tem precedência sobre 🎨 Design system legacy
```

R2 do Carrossel passa a ser uma camada fina sobre o marca-raiz — só específico de carrossel (espinha dorsal de slides, ritmo dark/light).

### Com agentes futuros (vídeo, ads, email, deck)

Mesma lógica. Cada novo agente:
1. Lê marca-raiz primeiro
2. Aplica seu pipeline específico
3. Outputs vão pro mesmo `🎯 Assets` database (com tipo diferente)

marca-raiz é a **camada-mãe** que todos compartilham.
