...
Toda integração do Prisma segue regra única: **conectores nativos do Claude Desktop**.
...
O sistema do marca-raiz cria mais coisas que o do Carrossel: **4 databases + 14 sub-páginas + 1 página principal**. Mais profundidade significa mais blocos, mais relations, mais cuidado com idempotência.

---

## Estrutura criada automaticamente pelo agente

Página principal: `🧬 {brand_name} — Prisma`
...
| Última revisão marca-raiz | Date | Quando esse touchpoint foi auditado contra o marca-raiz |

**Vistas:** "Por categoria" (Board, group by Categoria — padrão), "Por canal" (Table, ordem Frequência), "Sem revisão recente" (Table, filtro Última revisão marca-raiz < 90 dias atrás).
...
| Brief processado | Text | Brief enriquecido pelo R2 com marca-raiz aplicado |
...
Histórico de auditorias. Toda vez que R2 audita um asset (próprio ou externo) contra o marca-raiz, registra aqui. Cria histórico de aderência ao marca-raiz ao longo do tempo.
...
1. `🧬 marca-raiz` — conteúdo de `01-Marca-Raiz-Prisma.md`, populado com a síntese executiva do wizard. **A logo PNG é anexada aqui pelo usuário depois.**
...
5. `👥 Audience marca-raiz` — conteúdo de `06-Audience-marcaraiz.md`, populado com respostas da Fase 3
...
**Como agentes leem:** R2 do marca-raiz e R2 do Carrossel validam o ambiente no shell com `higgsfield account status`. Se falhar, param com erro claro pedindo instalação ou login.
...
- `notion-create-pages` cria página `🧬 {brand_name} — Prisma` no workspace adequado
...
### Passo 11 — Salvar `.marcaraiz.json`
- Snapshot completo do marca-raiz em JSON local (cache rápido pra subprocess)
- Schema na seção `🧬 marca-raiz` (`01-Marca-Raiz-Prisma.md`)
...
- [ ] `notion-ids.json` + `.brand.json` + `.marcaraiz.json` salvos no working folder
...
Se a marca também tem Carrossel instalado, o `notion-ids.json` é **mesclado** — contém também `page_brand_identity`, `db_news_feed`, `db_carrosseis`, etc., do Carrossel. R2 do Carrossel passa a ler `page_dna_master` antes de ler `page_brand_identity` (marca-raiz tem precedência).
...
O agente lê `.brand.json` + `.marcaraiz.json`, reaplica interpolação nos `.md`, e faz `PATCH /v1/blocks/{id}/children` substituindo os blocks de cada sub-página. **Não recria a estrutura, não toca nos databases, não duplica.** Útil quando você editou um `.md` localmente, ou quando o conteúdo do Notion saiu de sync com o JSON local.

```bash
> rebuild marca-raiz --pages=voice,visual
```
...
R2 cria automaticamente um snapshot do marca-raiz inteiro a cada 90 dias, salvo no Drive em `{brand_slug}/marcaraiz-Snapshots/{YYYY-MM-DD}/`:

- `marcaraiz-master.pdf` — export da página `🧬 marca-raiz`
- `marcaraiz-full.zip` — export de todas as 14 páginas + 4 databases
- `marcaraiz.json` — versão estruturada
- `changelog.md` — diff vs snapshot anterior

Vale como histórico ("como era nosso marca-raiz em Q1 vs Q3") e como backup. Pra rodar manual: comando `> snapshot marca-raiz` no agente CLI.
