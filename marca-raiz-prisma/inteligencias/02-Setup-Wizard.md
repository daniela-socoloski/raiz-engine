...
> **Não é página do Notion.** Esse é o roteiro que o **agente de setup (Claude Code CLI)** executa na primeira interação pra coletar o marca-raiz da marca em 7 fases de discovery, antes de criar a estrutura no Notion.
>
> O wizard do marca-raiz é mais profundo que o do Carrossel — não são 11 perguntas, são **52 perguntas em 7 fases**. Roda em ~30-45 min. O resultado é um marca-raiz propriamente dito, não um cadastro.
...
Primeira mensagem que você manda pode ser qualquer coisa — `oi`, `começar`, `?`. O Maestro (definido em `CLAUDE.md`) reconhece que não há `.marcaraiz.json` no working folder e abre conversação.
...
|---|---|---|
| **Lean** (12 perguntas) | ~10 min | Marca nova sem material existente, ou teste rápido. Output: marca-raiz bruto, suficiente pra começar |
| **Standard** (32 perguntas) | ~25 min | Marca em formação, com alguma intuição clara. Output: marca-raiz sólido pra produção |
| **Pro** (52 perguntas) | ~45 min | Marca operacional que quer formalizar marca-raiz. Output: marca-raiz completo, nível estúdio de branding |
| **Audit-first** (Pro + auditoria) | ~75-120 min | Marca operacional com material existente (site, Instagram, portfólio, blog, decks, posts, concorrentes, imprensa). Agente coleta links recursivamente, analisa material primeiro, devolve "marca-raiz observado", você confronta com "marca-raiz declarado" nas 52 perguntas |

> **Recomendação:** se você tem material público da marca, vai de **Audit-first**. O contraste entre "o que eu acho que somos" (declarado) e "o que parece ser" (observado) é onde o marca-raiz real aparece.
>
...
Se a pessoa mandar imagens que contêm texto, tabelas, listas ou exemplos de estrutura, o Maestro deve ler o conteúdo dessas imagens. Elas podem ser briefing, checklist ou escopo do marca-raiz. Não tratar automaticamente como referência estética.
...
Ao começar um marca-raiz, antes da análise profunda, o Maestro deve mostrar claramente o que vai extrair das referências e da marca. Isso serve para alinhar expectativa e mostrar o esforço real do trabalho.
...
- marca-raiz da pessoa;
- marca-raiz da marca/empresa;
...
O marca-raiz final precisa soar adequado ao modelo. Marca pessoal não deve virar relatório corporativo. Empresa não deve virar diário íntimo. Influencer não deve ser tratado como e-commerce frio. Fundador-led não deve apagar o fundador quando ele é o principal ativo de confiança.

### Quando a marca ainda não tem presença pública

Se não houver site, Instagram, portfólio, blog, loja, campanha publicada ou arquivos suficientes, o Maestro deve explicar que o marca-raiz continua possível, mas a base será construída por entrevista guiada e materiais alternativos.
...
- onde esse marca-raiz será aplicado: social, anúncio, landing page, apresentação, embalagem, vídeo, atendimento, evento ou proposta.

O objetivo é preencher as mesmas intenções de análise mesmo sem referência pública. A ausência de site não autoriza um marca-raiz raso. Ela obriga o Maestro a coletar mais contexto humano, registrar lacunas e separar três níveis no documento: observado, declarado pela pessoa e proposto como hipótese.
...
- diferença entre marca-raiz declarado, marca-raiz observado e marca-raiz performado
...
Quando o usuário enviar um site, o Maestro deve tratar esse site como fonte primária do marca-raiz. Site não é só URL. Site é arquitetura, narrativa, tom de voz, imagem, ritmo, prova, promessa e comportamento.
...
O Maestro não pode pular de "li os materiais" para "gerei o marca-raiz". Ele deve atravessar cinco camadas:
...
5. **Documentação final** — escrever o marca-raiz como uma história clara, com profundidade e base sólida, sem depender de produção de peças novas

Cada camada precisa deixar rastro no `discovery/` ou no `resultado/marcaraiz.md`. Se o marca-raiz só descreve o que a marca já tinha, falhou. O Maestro precisa interpretar, traduzir, organizar e documentar.
...
O logo deve entrar no `resultado/marcaraiz.pdf` sempre que a qualidade permitir. Se não houver logo, se estiver ilegível ou se a extração for ruim, o fluxo continua sem perda de qualidade. O Maestro registra o limite e usa nome, tipografia, cores e sinais visuais disponíveis.
...
- Que regra prática entra no marca-raiz?
- O que seria cópia ruim dessa referência?
- Como isso muda a forma de explicar voz, visual, comportamento ou posicionamento?

Quando o material enviado pedir estrutura de conteúdo, o Maestro deve transformar isso em matrizes claras. O marca-raiz deve conter, quando aplicável:
...
- `discovery/marcaraiz-observado.md` — síntese estratégica, verbal, visual e comportamental inferida
...
A camada do "pra quem". Sai daqui pra `👥 Audience marca-raiz`.
...
A camada do escopo editorial. Sai daqui pra `🧬 marca-raiz` (pilares) e `🚫 Anti-Patterns` (tabus).
...
```
Vou começar analisando o material existente da marca pra extrair o marca-raiz observado.
...
eu fecho o inventário, estudo tudo em profundidade e volto com o "marca-raiz observado".
...
13. **Síntese** → produz "marca-raiz observado" com variáveis preenchidas como **hipóteses rastreáveis**
...
```
marca-raiz OBSERVADO (extraído autonomamente):
...
Você confirma ou reescreve. **Esse contraste é onde marca-raiz real aparece** — frequentemente o que a marca declara é diferente do que ela faz.
...
```
═══ MARCA-RAIZ-PRISMA — RESUMO EXECUTIVO ═══
...
4. Cria `~/{brand-slug}/.marcaraiz.json` com o snapshot completo do marca-raiz (síntese executiva em JSON)
...
9. Popula `🧬 marca-raiz` com a síntese executiva
...
### Depois: R2 — marca-raiz Routine (Routine Local)

```
R1 criada. Agora a Routine 2 (R2 — marca-raiz Routine). 
New routine → tipo Local.
```
Solta no chat, um de cada vez: Nome → Working folder → Connectors/Tools → Schedule → Prompt completo.

Os prompts completos das Routines estão em `14-R1-Brand-Scout.md` e `15-R2-marcaraiz-Routine-Local.md` — o agente lê de lá e cola no chat (interpolando `{brand_*}`).
...
O marca-raiz gerado em modo Lean é **suficiente pra começar a produzir**, mas o resumo executivo da `🧬 marca-raiz` vem com tags `[expandir depois]` em vários campos. R2 reconhece essas tags e roda mini-discovery quando precisa de profundidade num campo `[expandir depois]`.
...
O agente apaga `.brand.json` + `.marcaraiz.json` + `.discovery-progress.json` e roda o wizard de novo.

Pra ajustar uma resposta específica sem refazer tudo:

```bash
> editar resposta da pergunta 27 do marca-raiz
```

Ou direto no Notion: edita a página relevante, depois roda `> rebuild marca-raiz` no agente — ele relê todas as páginas, regenera `.marcaraiz.json` e propaga interpolações.
...
> escrever email de boas-vindas seguindo marca-raiz    # gera output compliante
> qual nosso anti-pattern visual #2?            # consulta marca-raiz
> rebuild marca-raiz                                   # re-sincroniza .marcaraiz.json com Notion
> diff marca-raiz vs último snapshot                   # mostra o que mudou no marca-raiz
> snapshot marca-raiz                                  # cria snapshot trimestral no Drive
> revisar marca-raiz (quarterly)                       # roda mini-discovery (8 perguntas) pra refresh
```
