...
O marca-raiz é o `CLAUDE.md` da marca, expandido na profundidade de um manual de estúdio de branding (Pentagram, Wolff Olins, Collins, Porto Rocha, Manual). Captura propósito, estratégia, audiência, voz, visual, comportamento e anti-padrões num formato que IA lê e segue.

**White-label.** Sistema base não está atrelado a nenhuma marca. Toda variável da marca vive na página `🧬 marca-raiz` no Notion. Trocar a marca = trocar o conteúdo dessa página + as referências visuais. Nada no código muda.
...
Estúdios de branding entregam manuais de 200+ páginas porque é o que é necessário para que QUALQUER pessoa (designer, copy, atendimento, fundador) tome decisões coerentes sem voltar pro fundador toda vez. **O marca-raiz-prisma é isso, em formato lido por IA.**

A diferença prática:
- **Sem marca-raiz-prisma:** cada agente precisa de prompt enorme e ainda assim produz "ok-mas-genérico". Toda gestão de tom é manual. Cada novo agente precisa ser ensinado do zero.
- **Com marca-raiz-prisma:** todo agente lê o marca-raiz primeiro. Output sai com voz da marca, paleta correta, persona certa, anti-padrões respeitados. **Você plugou um cérebro de marca que vive fora dos prompts.**
...
| **R2 — marca-raiz Routine** | Routine **Local** (Claude Code Desktop) | Geração compliante (copy, imagem, deck) + auditoria de aderência (qualquer asset existente vs. marca-raiz) — precisa Higgsfield CLI, vision real, render |
...
A arquitetura espelha a do Carrossel — duas routines, working folder local, MCPs, Notion como fonte da verdade — mas o marca-raiz é uma camada-mãe. Quando o sistema de Carrossel roda, ele lê o marca-raiz do Notion ANTES de ler suas próprias páginas. Quando você cria um sistema novo (vídeo, ads), ele também lê o marca-raiz.
...
- Wizard discovery profundo (52 perguntas em 7 fases) extrai marca-raiz bruto
- Auditoria opcional: o agente analisa Instagram/site/decks da marca pra extrair marca-raiz observado
- Síntese: agente confronta marca-raiz declarado (perguntas) com marca-raiz observado (auditoria) e devolve um draft pra você refinar
- Após aprovação, popula 14 páginas no Notion + cria 4 databases + ativa 2 routines

**2. Uso passivo (toda outra inteligência)**
- Sistema Carrossel lê `🧬 marca-raiz` antes de cada execução, hidrata variáveis no pipeline editorial e nos prompts de render
- Sistema Vídeo, Ads, Email — qualquer agente futuro — lê do mesmo lugar
- marca-raiz é fonte única de verdade. Edita no Notion, próxima execução de qualquer agente já reflete

**3. Uso ativo (R2 marca-raiz Routine)**
- **Modo gerar:** "preciso de uma copy de email de boas-vindas" → R2 lê marca-raiz, escreve, devolve aderente
...
- **Modo evoluir:** "estamos repensando posicionamento" → R2 conduz mini-discovery, propõe diff no marca-raiz, aplica após aprovação
...
## Configuração do marca-raiz

Tudo via Notion. A R2 e qualquer agente derivado lê estas páginas:

- `🧬 marca-raiz` — variáveis canônicas + síntese executiva (1 página, leitura rápida)
- `🎯 Brand Strategy` — propósito, missão, visão, posicionamento, key insight
- `👥 Audience marca-raiz` — persona primária + secundária + anti-persona, jobs-to-be-done
...
4. `15-R2-marcaraiz-Routine-Local.md` — coração do sistema: prompt completo da Routine Local
...
O marca-raiz Prima **não substitui** a `🏷️ Brand Identity` do Carrossel — **alimenta**. A relação:

| Página Carrossel (`01. Carrossel`) | Origem no marca-raiz-prisma |
|---|---|
| `🏷️ Brand Identity` (variáveis: nome, handle, cor, etc.) | Lê de `🧬 marca-raiz` (single source of truth) |
...
Quando você instala o marca-raiz-prisma numa marca que já tem Carrossel rodando:
1. Setup cria estrutura marca-raiz num workspace separado (ou na mesma)
2. R2 do Carrossel passa a ler do marca-raiz antes de ler suas próprias páginas
3. Edits no marca-raiz propagam pro Carrossel automaticamente

Quando você instala o marca-raiz-prisma do zero (sem Carrossel ainda):
1. Setup cria a estrutura marca-raiz primeiro
2. Quando depois você instalar o Carrossel, ele já encontra o marca-raiz e usa
...
├── .marcaraiz.json                      # síntese executiva do marca-raiz (cache rápido)
...
│   └── {brand_name} — marca-raiz Routine (Local)
└── Connectors
    ├── Notion
    └── Google Drive

Notion workspace
└── 🧬 {brand_name} — marcaraizprisma/
    ├── 🧬 marca-raiz              ← ponto de entrada, single source of truth
    ├── 🎯 Brand Strategy
    ├── 👥 Audience marca-raiz
    ├── 🗣️  Voice & Tone
    ├── 🎨 Visual System
...
Google Drive
└── {brand-slug}/
    ├── marcaraiz-Snapshots/{YYYY-MM-DD}/  ← snapshot trimestral do marca-raiz inteiro
...
**3. O marca-raiz é vivo.** Marca evolui. O marca-raiz evolui junto. Trimestralmente, R2 sugere atualizações baseadas em performance (que conteúdo está performando, quais decisões editoriais resultaram em alcance). A `🔍 Discovery Protocol` é re-rodada anualmente pra refresh maior. **Não é manual congelado de 2026 — é organismo vivo.**
