...
Diagnóstico dos problemas mais comuns. Sistema marca-raiz-prisma V1.0 — Routines do Claude Desktop.
...
- **Solução:** agente já adiciona `time.sleep(0.4)` entre batches. Se ainda assim falhar, roda `rebuild marca-raiz` no agente — ele detecta gaps e completa.

### Páginas duplicadas após setup
- **Causa:** criação rodou em background + foreground simultâneos (bug clássico)
- **Solução:** apague as duplicatas manualmente no Notion, depois rode `rebuild marca-raiz`. Garante que o `.brand.json` aponta pros IDs corretos.
...
- **Solução:** edita `🧬 marca-raiz` → seção "Estética-âncora" → preencha com 2-3 referências específicas (Eye Magazine, Dia Studio, etc.). Próxima run usa.
...
- **Solução:** se (1), revise critério de aprovação na curadoria semanal. Se (2), considere rodar `evolve` modo (b) Refresh visual no R2 do marca-raiz.
...
## R2 (marca-raiz Routine — Routine Local)
...
- **Causa 1 (alto):** marca-raiz pouco definido — agente não tem como discriminar
...
- **Causa:** essa dimensão precisa de mais densidade no marca-raiz
...
### R2 sugere mudança no marca-raiz mas você discorda (modo Evolve)
- **Sintoma:** rodou evolve, R2 propôs diff, mas algo não bate com sua visão
- **Solução:** opção (e) Editar — abre cada item do diff individualmente, ajusta. Ou cancele a aplicação — marca-raiz fica intacto.

### Evolve aplicou mudança e quero reverter
- **Sintoma:** apliquei evolve mas resultado ficou ruim
- **Solução:** snapshot pré-evolução está em Drive `{brand_slug}/marcaraiz-Snapshots/{TODAY}-pre-evolve/`. Use os arquivos de lá pra restaurar:
  ```bash
  cd ~/{brand-slug}
  claude
  > restaurar snapshot marca-raiz de {data}
  ```
...
### `.marcaraiz.json` saiu de sync com Notion
- **Sintoma:** você editou página no Notion, mas R2 ainda usa versão antiga
- **Causa:** `.marcaraiz.json` é cache local
- **Solução:**
  ```bash
  cd ~/{brand-slug}
  claude
  > rebuild .marcaraiz.json
  ```
  Agente lê todas as páginas marca-raiz do Notion, regenera JSON local. ~30s.

---

## Notion (operacional, pós-setup)

### `🧬 marca-raiz` mostrando `{brand_*}` literalmente (sem interpolar)
- **Causa:** página foi populada por humano (sem interpolar) ou rebuild bugou
- **Solução:** `> rebuild marca-raiz --pages=dna_master` no agente. Re-faz interpolação.
...
### R2 do Carrossel não está lendo o marca-raiz
- **Sintoma:** Carrossel sai com tom antigo, ignorando atualizações do marca-raiz
- **Causa:** prompt da Routine R2 do Carrossel não foi atualizado pra incluir Etapa -1 (carregar marca-raiz)
...
### Conflito entre Brand Identity (Carrossel) e marca-raiz (marca-raiz)
- **Sintoma:** as duas páginas têm versões diferentes da mesma variável (ex: cor primária diferente)
- **Solução:** marca-raiz é fonte da verdade. Esvazie a página `🏷️ Brand Identity` do Carrossel deixando só uma nota:
  ```
  ⚠️ Esta página é legacy. Variáveis canônicas vivem agora em 🧬 marca-raiz.
  R2 do Carrossel lê marca-raiz primeiro.
  ```
...
7. `.marcaraiz.json` reflete o Notion? → `cat .marcaraiz.json | jq .` e compara com `🧬 marca-raiz`
...
|---|---|
| Brand book congelado fica sem uso → marca-raiz tem que ser executável | Sistema todo é "lido por IA, não por humano só" |
| Discovery superficial gera marca-raiz superficial → 52 perguntas com critério de qualidade | Wizard Pro força especificidade |
...
| marca-raiz precisa ser organismo, não monumento | R1 alimenta refs continuamente; auto-snapshot trimestral |
| Compliance subjetivo gera retrabalho | Compliance score quantitativo (0-10 por dimensão) |
| Onboarding de novo membro leva semanas → marca-raiz explícito acelera | Protocolo de onboarding em 7 dias |
| Concorrente sem monitoria gera diferenciação fraca | R1 monitora 3 concorrentes mensalmente |
| Persona genérica gera copy genérica → persona como personagem de romance | `👥 Audience marca-raiz` exige bio densa de 6-10 linhas |
...
| Conflito entre Brand Identity legacy e marca-raiz → resolver com precedência | marca-raiz é canônico; Brand Identity vira legacy |
| Mac pode estar offline | R2 marca-raiz é manual (não tem schedule diário); R1 tem catch-up |
