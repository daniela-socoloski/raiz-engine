...
## Cenário 1 — Produzir asset compliante seguindo o marca-raiz
...
1. Abre Claude Desktop → Routines → `{brand_name} — marca-raiz Routine` → **Run now**
2. Primeira mensagem:
   ```
...
3. R2 entra em MODO GENERATE:
   - Lê marca-raiz (voz, audience, behavior, anti-patterns)
...
2. R2 entra em MODO AUDIT:
   - Lê marca-raiz (voz, visual, audience, anti-patterns)
...
## Cenário 3 — Mini-discovery de evolução (marca-raiz precisa atualizar)

Situação: a marca pivotou produto, ou audiência amadureceu, ou estética envelheceu. marca-raiz precisa refresh.
...
4. R2 apresenta diff proposto:
   ...
   Páginas afetadas: 🧬 marca-raiz, 🎨 Visual System, 📚 Reference Library
...
6. **Próxima execução de qualquer agente** (R2 Carrossel, futuros agentes) já lê o marca-raiz atualizado
...
Situação: vai contratar designer pra projeto pontual. Quer que a entrega chegue alinhada com o marca-raiz.

**Fluxo:**

1. Acesse no Notion: `🧬 marca-raiz`, `🎨 Visual System`, `📚 Reference Library`, `🚫 Anti-Patterns`

2. Compartilhe **as 4 páginas** com o fornecedor (Notion permite share de página específica)

3. Anexe:
   - PDF com as variáveis canônicas (export da `🧬 marca-raiz`)
...
4. Brief do projeto inclui contrato simples:
   > "Toda entrega passa por compliance check via R2 do marca-raiz Routine. Score < 7 em qualquer dimensão = revisão sem custo extra. Ajustes contemplados no escopo até score ≥ 8."
...
Situação: entrou alguém novo (designer, copy, social, atendimento). Quer que produza dentro do marca-raiz desde a primeira semana.
...
- Atendimento → 🤝 Brand Behavior + 👥 Audience marca-raiz
...
- Roda 8 perguntas pessoais: o que mudaria no marca-raiz com base no que viu nos 30 dias?
...
5. Você lê em reunião estratégica trimestral. Decisões podem virar `evolve` no marca-raiz.

---

## Cenário 7 — Rodar discovery anual completo (refresh do marca-raiz)

Situação: passou 1 ano. Marca evoluiu. marca-raiz precisa de revisão sistemática.

**Fluxo:**

1. Cria snapshot completo do marca-raiz atual (segurança):
   ```bash
   cd ~/{brand-slug}
   claude
   > snapshot marca-raiz quarterly-Q[N]-YYYY
   ```
...
4. Após confirmação final:
   - Cria diff completo (marca-raiz antes vs. depois)
   - Cria snapshot etiquetado: `marca-raiz Q[N]-{YYYY} — pre-anual-refresh`
   - Aplica todas as mudanças em batch

5. Comunicado interno (opcional):
   - Agente gera changelog "o que mudou no marca-raiz neste refresh anual"
   - Time lê
...
2. R2 entra em MODO GENERATE:
   ...
   - Compara scores e sinaliza qual é mais aderente ao marca-raiz
...
### Snapshot trimestral do marca-raiz inteiro

R2 cria automaticamente em `{brand_slug}/marcaraiz-Snapshots/{YYYY-MM-DD}/` no Drive. Vale como histórico ("como era o marca-raiz em Q1 vs Q3") e como backup.
...
5. **Por último:** `🧬 marca-raiz` (variáveis canônicas)
...
```text
# Day-to-day (no painel da Routine Local — marca-raiz Routine)
...
cat ~/{brand-slug}/.marcaraiz.json | jq .          # snapshot completo
...
Routines → marca-raiz Routine → Run now                 # disparar agora
...
> snapshot marca-raiz                                 # snapshot trimestral manual
> diff marca-raiz vs último snapshot                  # mostra o que mudou
> análise competitiva trimestral
> rodar discovery completo (revisão anual)
```
...
- Quer fazer manutenção / debug do marca-raiz
...
| Discovery anual completo (refresh marca-raiz) | Anual | Time + fundador | 2-3h |
