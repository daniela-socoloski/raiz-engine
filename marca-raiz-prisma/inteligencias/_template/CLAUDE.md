...
```text
resultado/marcaraiz.md
```

O `marcaraiz.md` é a fonte da verdade. Ele define estilo visual, tom de voz, referências, anti-referências, ferramentas, workflow e critérios de qualidade. O `resultado/marcaraiz.pdf` é a versão editorial apresentável. Este `CLAUDE.md` é só a camada operacional para o Claude Code.

---

## Resumo da marca

Preencha este bloco a partir do `marcaraiz.md` quando o projeto for criado.
...
1. Leia o `marcaraiz.md` inteiro antes de responder.
2. Antes de criar, identifique qual seção do marca-raiz controla o pedido: visual, voz, audiência, workflow, canal, imagem ou comportamento.
3. Use as regras do marca-raiz para criar, revisar ou refinar peças.
4. Se o usuário pedir algo que contradiz o marca-raiz, sinalize a tensão e proponha um caminho coerente.
5. Se o usuário aprovar uma mudança de estilo, voz ou workflow, atualize o `marcaraiz.md`.
6. Depois de qualquer ajuste relevante, regenere `resultado/marcaraiz.pdf`.
...
Substitua os placeholders por síntese específica do marca-raiz.
...
```text
crie um post seguindo meu marca-raiz
escreva um email no tom da marca
audite esta peça contra o marca-raiz
ajuste o tom para ficar menos formal
refine o marca-raiz com este feedback
teste meu marca-raiz com uma peça pequena
```

## Regra de teste

Quando o marca-raiz for recém-criado, faça uma peça pequena para validar aderência:
...
Depois pergunte o que ficou certo e o que ficou fora. Feedback aprovado vira ajuste no `marcaraiz.md`.
...
- `resultado/marcaraiz.md` existe;
- `resultado/marcaraiz.pdf` existe;
