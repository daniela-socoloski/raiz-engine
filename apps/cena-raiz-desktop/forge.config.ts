...
const windowsSignToolPath = process.env.CENA_RAIZ_WIN_SIGNTOOL?.trim();
const windowsSignParams = process.env.CENA_RAIZ_WIN_SIGN_PARAMS?.trim();
const windowsSign =
...
// CENA_RAIZ_APPLE_ID (e-mail da conta), CENA_RAIZ_APPLE_APP_PASSWORD (senha de app
// gerada em appleid.apple.com) e CENA_RAIZ_APPLE_TEAM_ID. Sem elas o build segue
// local (ad-hoc), como sempre.
const appleId = process.env.CENA_RAIZ_APPLE_ID?.trim();
const appleAppPassword = process.env.CENA_RAIZ_APPLE_APP_PASSWORD?.trim();
const appleTeamId = process.env.CENA_RAIZ_APPLE_TEAM_ID?.trim();
const canNotarize = Boolean(macSigningIdentity && appleId && appleAppPassword && appleTeamId);

const config: ForgeConfig = {
  packagerConfig: {
    appBundleId: 'com.danielasocoloski.cena-raiz',
  ...
  makers: [
    new MakerSquirrel({
      name: 'cena-raiz',
      setupIcon: path.resolve('src/brand/cena-raiz-icon.ico'),
      // electron-winstaller 5.4+ assina Update.exe/Setup.exe com o mesmo
      // windowsSign do packager (que ja assinou o cena-raiz.exe do app).
      ...(windowsSign ? { windowsSign } : {}),
    }),
    new MakerDMG(
      {
        background: path.resolve('src/brand/dmg-background.png'),
        icon: path.resolve('src/brand/cena-raiz-icon.icns'),
        iconSize: 104,
        contents: (options) => [
          {
            x: 180,
  ...
  plugins: [
    ...
    new FusesPlugin({
      ...
      // cena-raiz does not persist Electron cookies. Keeping this fuse disabled avoids
