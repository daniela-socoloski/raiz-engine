...
import cenaRaizIcon from './brand/cena-raiz-icon.png';
import cenaRaizLogo from './brand/cena-raiz-logo.png';
...
function friendlyAiError(raw: string): string {
  ...
  if (/model is not supported|unsupported model|model_not_found|invalid model/iu.test(text)) {
    return 'O modelo de IA desta conta mudou e esta versão do cena-raiz ainda não o acompanha. Atualize o cena-raiz e tente de novo.';
  }
  return text;
}
...
function styleStorageKey(directory: string): string {
  return `cena-raiz:style:${directory}`;
}
...
function chatStorageKey(directory: string): string {
  return `cena-raiz:chat:${directory}`;
}
...
function EditorWorkspace({
  ...
  useEffect(() => {
    for (const clip of voiceClips) {
      ...
      void window.cenaRaizDesktop.getSourceWaveform(url)
  ...
  return (
    <div className="member-gate">
      <section className="member-card">
        <img className="member-logo" src={cenaRaizLogo} alt="cena-raiz" />
        {auth.status === 'no-access' ? (
          <>
            <h1>Sua matrícula não está ativa</h1>
            <p>
              O cena-raiz é liberado para membros com assinatura ativa no Sistema Marca
              Raiz. A conta {auth.email ? <strong>{auth.email}</strong> : 'informada'} não tem essa
              matrícula no momento — se você acabou de comprar ou renovou, feche e reabra o aplicativo.
            </p>
            <p className="member-hint">Dúvidas? Fale com o suporte do Sistema Marca Raiz.</p>
            <button type="button" className="btn ghost" onClick={() => void onLogout()}>Entrar com outra conta</button>
          </>
        ) : (
          <>
            <h1>Entre com sua conta</h1>
            <p>Use o mesmo e-mail e senha da sua área de membros.</p>
...
export function App() {
  ...
  const [railPinned, setRailPinned] = useState(() => localStorage.getItem('cena-raiz:rail-pinned') === 'true');
  ...
  async function refreshRuntimes() {
    setChecking(true);
    try {
      setRuntimes(await window.cenaRaizDesktop.checkRuntimes());
    } finally {
      setChecking(false);
    }
  }
  ...
  async function reloadProjects() {
    setProjects(await window.cenaRaizDesktop.listRecentProjects());
  }

  async function chooseProjectDirectory(name?: string) {
    setOpeningProject(true);
    try {
      const selected = await window.cenaRaizDesktop.selectProjectDirectory(name);
      if (selected) {
  ...
  async function confirmNameDialog() {
    ...
    if (!name) return;
    try {
      setProjects(await window.cenaRaizDesktop.renameProject(dialog.directory, name));
  ...
  async function togglePinProject(project: ProjectSummary) {
    setProjectMenu(null);
    try {
      setProjects(await window.cenaRaizDesktop.pinProject(project.directory, !project.pinned));
    } catch (error) {
      setMessages((current) => [...current, { id: `error:${Date.now()}`, role: 'system', text: errorMessage(error) }]);
    }
  }
  ...
  async function removeProjectFromList(project: ProjectSummary) {
    ...
    try {
      ...
      setProjects(await window.cenaRaizDesktop.removeRecentProject(project.directory));
    } catch (error) {
      setMessages((current) => [...current, { id: `error:${Date.now()}`, role: 'system', text: errorMessage(error) }]);
    }
  }
  ...
  async function openRecentProject(project: ProjectSummary) {
    ...
    setOpeningProject(true);
    try {
      activateWorkspace(await window.cenaRaizDesktop.openRecentProject(project.directory));
      await reloadProjects();
    } catch (error) {
      setMessages((current) => [...current, { id: `error:${Date.now()}`, role: 'system', text: errorMessage(error) }]);
    } finally {
  ...
  async function flushPendingTimelineSave() {
    ...
    try {
      await window.cenaRaizDesktop.saveTimelineModel(pending.directory, pending.model, pending.loadStamp);
    } catch (error) {
      setMessages((current) => [...current, { id: `error:${Date.now()}`, role: 'system', text: errorMessage(error) }]);
    }
  }
  ...
  async function refreshWorkspace() {
    ...
    try {
      ...
      const refreshed = await window.cenaRaizDesktop.refreshProjectWorkspace(directory);
  ...
  function requestPhase2Render() {
    ...
    void window.cenaRaizDesktop.renderPhase2(directory).catch(() => {});
  }
  ...
  function requestImageFulfillment() {
    ...
    void window.cenaRaizDesktop.fulfillImageRequests(directory).catch(() => {});
  }
  ...
  function requestJcutSync() {
    const directory = activeProjectDirectoryRef.current;
    if (!directory) return;
    void window.cenaRaizDesktop.syncJcut(directory).then((result) => {
      if (result?.changed) {
  ...
  async function login() {
    ...
    try {
      setAccount(await window.cenaRaizDesktop.loginWithChatGPT());
    } catch (error) {
  ...
  async function logout() {
    try {
      setAccount(await window.cenaRaizDesktop.logoutCodex());
  ...
  async function cancelLogin() {
    try {
      setAccount(await window.cenaRaizDesktop.cancelChatGPTLogin());
    } catch (error) {
  ...
  async function claudeLogin() {
    try {
      setClaudeAccount(await window.cenaRaizDesktop.loginWithClaude());
    } catch (error) {
  ...
  async function claudeCancelLogin() {
    setClaudeCode('');
    try {
      setClaudeAccount(await window.cenaRaizDesktop.cancelClaudeLogin());
    } catch (error) {
      setClaudeAccount({ status: 'error', email: null, error: errorMessage(error) });
    }
  }
  ...
  async function claudeLogout() {
    try {
      setClaudeAccount(await window.cenaRaizDesktop.logoutClaude());
    } catch (error) {
  ...
  async function claudeSubmitCode() {
    ...
    if (!code) return;
    try {
      setClaudeAccount(await window.cenaRaizDesktop.submitClaudeLoginCode(code));
      setClaudeCode('');
    } catch (error) {
      setClaudeAccount({ status: 'error', email: null, error: errorMessage(error) });
    }
  ...
  function switchAiProvider(provider: AiProvider, pinned = true) {
    void window.cenaRaizDesktop.setAiRole('chat', provider, pinned).then(setAiRoles);
  }

  function switchImageProvider(provider: AiProvider | null, pinned = true) {
    void window.cenaRaizDesktop.setAiRole('image', provider, pinned).then(setAiRoles);
  }
  ...
  async function submitProviderKey() {
    ...
    try {
      if (keyEntry === 'chatgpt') {
        const state = await window.cenaRaizDesktop.loginCodexWithApiKey(apiKey);
        setAccount(state);
        if (state.status === 'signed-in') setKeyEntry(null);
      } else if (keyEntry === 'claude') {
        const state = await window.cenaRaizDesktop.connectClaudeApiKey(apiKey);
        setClaudeAccount(state);
        if (state.status === 'signed-in' && state.mode === 'api-key') setKeyEntry(null);
      } else {
        const state = await window.cenaRaizDesktop.connectGeminiApiKey(apiKey);
  ...
  async function dispatchMessage(text: string, displayText = text) {
    ...
    try {
      const result = await window.cenaRaizDesktop.sendCodexMessage({ projectDirectory, text: trimmed });
  ...
  async function applyStyleSelection() {
    ...
    const runtime = await window.cenaRaizDesktop.ensureRemotionRuntime();
    ...
    try {
      await window.cenaRaizDesktop.scaffoldRemotionProject(projectDirectory);
    } catch (error) {
      setMessages((current) => [...current, { id: `error:${Date.now()}`, role: 'system', text: errorMessage(error) }]);
      return;
    }
    ...
    const prompt = [
      ...
      ...(style.edit === 'split' || style.edit === 'split2'
        ? [
            '',
            'Tela dividida — regra de conteúdo: por padrão, GERE IMAGENS com IA ilustrando o que está sendo dito em cada trecho da fala. Peça as imagens em edit/imagens/pedidos.json (o cena-raiz gera os arquivos em edit/imagens/) e use cada arquivo no campo splits do edit-data.json, cobrindo os principais trechos do vídeo com a imagem correspondente ao assunto daquele momento.',
        ...
        : []),
      '',
      'O cena-raiz já montou o projeto Remotion em edit/remotion, com as dependências instaladas.',
      ...
      'Não execute remotion render: quando os dados estiverem prontos, encerre o turno com um resumo curto — o cena-raiz renderiza sozinho e mostra o progresso na interface.',
  ...
  async function applyJcut() {
    ...
    setJcutBusy(true);
    try {
      const result = await window.cenaRaizDesktop.applyJcut(projectDirectory);
      if (result.applied) {
  ...
  function handleTimelineModelChange(model: TimelineModel, commit: boolean) {
    ...
    timelineSaveTimerRef.current = window.setTimeout(() => {
      ...
      if (!pending) return;
      pendingTimelineSaveRef.current = null;
      void window.cenaRaizDesktop.saveTimelineModel(pending.directory, pending.model, pending.loadStamp).catch((error) => {
        setMessages((current) => [...current, { id: `error:${Date.now()}`, role: 'system', text: errorMessage(error) }]);
      });
    }, 400);
  }
  ...
  async function applyTimelineEdits() {
    ...
    const prompt = [
      'Editei a timeline diretamente no cena-raiz Desktop (trims, cortes e exclusões de takes).',
  ...
  async function interruptTurn() {
    ...
    try {
      await window.cenaRaizDesktop.interruptCodexTurn(activeTurn.threadId, activeTurn.turnId);
    } catch (error) {
      setMessages((current) => [...current, { id: `error:${Date.now()}`, role: 'system', text: errorMessage(error) }]);
    }
  }
  ...
  async function answerApproval(approval: CodexApproval, decision: 'accept' | 'acceptForSession' | 'decline') {
    ...
    setApprovalError(null);
    try {
      await window.cenaRaizDesktop.respondToCodexApproval(approval.id, decision);
    } catch (error) {
  ...
  function toggleRailPinned() {
    setRailPinned((current) => {
      localStorage.setItem('cena-raiz:rail-pinned', String(!current));
      return !current;
    });
  }

  useEffect(() => {
    const unsubscribe = window.cenaRaizDesktop.onCodexEvent(handleCodexEvent);
    const unsubscribeModel = window.cenaRaizDesktop.onWhisperModelState(setWhisperModel);
    const unsubscribeRemotion = window.cenaRaizDesktop.onRemotionRuntimeState(setRemotionRuntime);
    const unsubscribePhase2 = window.cenaRaizDesktop.onPhase2RenderState(handlePhase2RenderState);
    const unsubscribeUpdate = window.cenaRaizDesktop.onAppUpdateState(setAppUpdate);
    const unsubscribeMember = window.cenaRaizDesktop.onMemberAuthState(setMemberAuth);
    const unsubscribeClaude = window.cenaRaizDesktop.onClaudeAccount(setClaudeAccount);
    const unsubscribeGemini = window.cenaRaizDesktop.onGeminiAccount(setGeminiAccount);
    const unsubscribeRoles = window.cenaRaizDesktop.onAiRoles(setAiRoles);
    const unsubscribeImageGen = window.cenaRaizDesktop.onImageGenState(handleImageGenState);
    void window.cenaRaizDesktop.getMemberAuth().then(setMemberAuth);
    const unsubscribePack = window.cenaRaizDesktop.onRuntimePackState((state) => {
    ...
    void window.cenaRaizDesktop.ensureRuntimePack().then(setRuntimePack);
    if (!booted.current) {
      booted.current = true;
      void window.cenaRaizDesktop.getDesktopInfo().then(setDesktopInfo);
      void window.cenaRaizDesktop.getCodexAccount().then(setAccount);
      void window.cenaRaizDesktop.getClaudeAccount().then((state) => {
        setClaudeAccount(state);
        setClaudeLoaded(true);
      });
      void window.cenaRaizDesktop.getGeminiAccount().then((state) => {
        setGeminiAccount(state);
        setGeminiLoaded(true);
      });
      void window.cenaRaizDesktop.getAiRoles().then(setAiRoles);
      ...
      void window.cenaRaizDesktop.ensureWhisperModel().then(setWhisperModel);
      void window.cenaRaizDesktop.listRecentProjects().then(async (recent) => {
        setProjects(recent);
        if (recent[0]) {
          try {
            activateWorkspace(await window.cenaRaizDesktop.openRecentProject(recent[0].directory));
            setProjects(await window.cenaRaizDesktop.listRecentProjects());
          } catch {
  ...
  // IA estiver conectada, o cena-raiz oferece ChatGPT, Claude e Gemini. O clique
  ...
  const aiModal = showAiOnboarding && (
    <div className="modal-overlay ai-overlay">
      <section className="ai-onboarding" role="dialog" aria-modal="true" aria-label="Conecte sua IA">
        ...
        <p>O cena-raiz edita com a inteligência da sua própria conta. Escolha qual conectar — dá para trocar depois em Configurações.</p>
  ...
  const packModal = (runtimePack.status === 'downloading' || runtimePack.status === 'extracting' || runtimePack.status === 'error') && (
    <div className="pack-overlay" role="dialog" aria-modal="true" aria-label="Preparando o cena-raiz">
      <div className="pack-modal">
        <strong>Preparando o cena-raiz</strong>
        {runtimePack.status === 'error' ? (
          <>
            <p className="pack-error">{runtimePack.error ?? 'Não foi possível baixar as ferramentas.'}</p>
            <button type="button" className="btn primary" onClick={() => void window.cenaRaizDesktop.ensureRuntimePack().then(setRuntimePack)}>
              Tentar de novo
            </button>
          </>
        ) : (
      ...
      <aside className={`project-rail ${railPinned || projectMenu ? 'pinned' : ''}`}>
        <div className="rail-brand">
          <div className="rail-logo">
            <img className="rail-brand-icon" src={cenaRaizIcon} alt="" />
            <img className="rail-brand-wordmark" src={cenaRaizLogo} alt="cena-raiz" />
          </div>
        ...
        <div className="rail-footer">
          <div className="rail-account">
            ...
            <span className="rail-account-copy">
              <strong>{memberAuth.name ?? memberAuth.email ?? 'cena-raiz'}</strong>
              <small>{memberAuth.status === 'signed-in' ? memberAuth.email : 'Sistema Marca Raiz'}</small>
            </span>
          ...
          {workspace && (
            <div className="project-subcard">
              ...
              <button
                ...
                onClick={() => void window.cenaRaizDesktop.openProjectFolder(workspace.project.directory).catch(() => {})}
        ...
        <div className="studio-grid">
          <section className="chat-panel">
            <div className="chat-head">
              <div><span className="eyebrow">Direção da edição</span><h1>Converse com o cena-raiz</h1></div>
                ...
                <div className="chat-empty compact">
                  ...
                  {whisperModel.status === 'error' && (
                    <div className="model-status error">
                      ...
                      <button type="button" className="btn ghost small" onClick={() => void window.cenaRaizDesktop.ensureWhisperModel().then(setWhisperModel)}>
                        Tentar de novo
                      </button>
                    </div>
                  )}
              ...
              {messages.map((message) => {
                ...
                return (
                  <article className={`chat-message ${message.role}`} key={message.id}>
                    <span className="chat-role">{message.role === 'user' ? 'Você' : message.role === 'assistant' ? 'cena-raiz' : 'Sistema'}</span>
                    <p>{visibleText || '...'}</p>
                    {showsCutApproval && (
        ...
        <div className="approval-overlay">
          ...
          >
            ...
            <p id="approval-description" className="approval-description">
              O cena-raiz precisa desta autorização para continuar a tarefa. O processo ficará pausado até você decidir.
            </p>
      ...
      {nameDialog && (
        <div className="modal-overlay" onClick={(event) => { if (event.target === event.currentTarget) setNameDialog(null); }}>
          <section className="name-dialog" role="dialog" aria-modal="true">
            ...
            <p>{nameDialog.mode === 'create' ? 'Dê um nome ao projeto e escolha a pasta com o vídeo.' : 'O novo nome aparece na lista e no topo do cena-raiz.'}</p>
            <input
              type="text"
              value={nameValue}
              autoFocus
          ...
          <section className="settings-modal" role="dialog" aria-modal="true" aria-label="Configurações">
            ...
            {settingsTab === 'geral' ? (
                ...
                <div className="settings-block">
                  ...
                  {memberAuth.status === 'signed-in' ? (
                    <div className="settings-row">
                      ...
                      <button type="button" className="account-action" onClick={() => void window.cenaRaizDesktop.memberLogout().then(setMemberAuth)}>Sair</button>
                    </div>
                  ) : (
              ...
              <div className="settings-body">
                <div className="settings-block">
                  ...
                  <div className="settings-row">
                    ...
                    <div className="settings-row-actions">
                      ...
                      {geminiConnected
                        ? <button type="button" className="account-action" onClick={() => void window.cenaRaizDesktop.disconnectGemini().then(setGeminiAccount)}>Sair</button>
                ...
                <div className="settings-block">
                  <h3>MCPs</h3>
                  <p className="settings-note">Em breve: conexões MCP para ampliar o cena-raiz.</p>
                </div>
              </div>
            )}
          </section>
