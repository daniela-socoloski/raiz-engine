...
function qaProject(): ProjectSummary | null {
  const directory = process.env.CENA_RAIZ_QA_PROJECT_PATH?.trim();
...
// userData/runtime/tools. Cada release do CenaRaiz volta a pesar ~100 MB.
...
async function requireRuntimePack(): Promise<void> {
  ...
  if (state.status !== 'ready') {
    throw new Error(state.error || 'As ferramentas do CenaRaiz ainda estão sendo preparadas.');
  }
}
...
async function inspectProjectMedia(directory: string): Promise<InspectedProjectMedia | null> {
  ...
  return {
    ...
    media: {
      url: `cena-raiz-media://local/${token}`,
  ...
  for (const candidatePath of candidatePaths) {
    try {
      ...
      return {
        ...
        accent: /^#[0-9a-f]{6}$/iu.test(style.accent ?? '') ? style.accent as string : '#09b5b7',
        elements: {
...
// autorizada (cena-raiz-media://), entao nao ha resolucao de caminho nova aqui.
...
async function readSourceWaveform(mediaUrl: string): Promise<SourceWaveform | null> {
  ...
  try {
    ...
    if (url.protocol !== 'cena-raiz-media:' || url.hostname !== 'local') return null;
...
const FONTS_CSS_VERSION = 'CenaRaiz fonts v2 (woff2 embutido)';
...
async function downloadRemotionFonts(fontsDirectory: string): Promise<void> {
  ...
  await writeFile(
    ...
    `/* ${FONTS_CSS_VERSION} — gerado pelo CenaRaiz Desktop para render offline. */\n${blocks.join('\n')}\n`,
  );
}
...
function ensureRemotionRuntime(): Promise<RemotionRuntimeState> {
  ...
  const pending = (async (): Promise<RemotionRuntimeState> => {
    ...
    await writeFile(
      ...
      `${JSON.stringify(
        {
          name: 'cena-raiz-remotion-runtime',
          version: '1.0.0',
          private: true,
...
// ferramentas empacotadas, variaveis CENA_RAIZ_* e caches fora do sandbox.
function agentToolsEnvironment(): NodeJS.ProcessEnv {
  ...
  return {
    ...
    PYTHONDONTWRITEBYTECODE: '1',
    PYTHONNOUSERSITE: '1',
    CENA_RAIZ_PYTHON: runtimeCommand('python'),
    CENA_RAIZ_FFMPEG: runtimeCommand('ffmpeg'),
    CENA_RAIZ_FFPROBE: runtimeCommand('ffprobe'),
    CENA_RAIZ_UV: runtimeCommand('uv'),
    CENA_RAIZ_YTDLP: runtimeCommand('yt-dlp'),
    ...
    HF_HUB_OFFLINE: '1',
    CENA_RAIZ_WHISPER_MODEL: WHISPERX_MODEL_NAME,
    ...
    CENA_RAIZ_HELPERS: helpersDirectory(),
  };
}
...
function applyJcutToProject(projectDirectory: string): Promise<JcutApplyResult> {
  ...
  const job = (async (): Promise<JcutApplyResult> => {
    ...
    if (!ffmpeg.command || !ffprobe.command) {
      return { applied: false, cuts: 0, error: 'As ferramentas de vídeo do CenaRaiz não estão disponíveis.' };
    }
    ...
    const workDirectory = await mkdtemp(path.join(os.tmpdir(), 'cena-raiz-jcut-'));
    try {
      const pieces: string[] = [];
...
// no auth.json que o proprio app-server guarda no CODEX_HOME do CenaRaiz.
...
function registerIpcHandlers(): void {
  ...
  ipcMain.handle('project:open-recent', async (_event, input: { directory?: string }) => {
    ...
    if (!isRecent) throw new Error('Este projeto nao esta na lista recente do CenaRaiz.');
  ...
  ipcMain.handle('codex:message', async (_event, input: CodexSendMessageInput) => {
    ...
    if (!selectedProjectDirectories.has(resolvedProjectDirectory)) {
      throw new Error('Escolha a pasta do projeto pelo seletor do CenaRaiz.');
    }
    if (!text) throw new Error('Escreva uma mensagem para o CenaRaiz.');
...
// --- Login de membro (Supabase proprio) ------------------------------------
// O usuario entra com e-mail/senha: autenticacao direta no Supabase Auth do
// Sistema Marca Raiz com a publishable key (chave publica, protegida pelas
// RLS). O direito de uso e uma matricula com status 'active' e nao expirada
// na tabela `enrollments`, lida por RLS que limita cada usuario as suas.
...
const MEMBER_SUPABASE_URL =
  process.env.CENA_RAIZ_SUPABASE_URL?.trim() ||
  // URL publica do projeto Supabase proprio do Sistema Marca Raiz.
  'https://chfrrgnuinhhkvavndsw.supabase.co';
const MEMBER_SUPABASE_ANON_KEY =
  process.env.CENA_RAIZ_SUPABASE_ANON_KEY?.trim() ||
  // Publishable key publica do projeto (a mesma que o site entrega ao
  // navegador; protegida pelas RLS — a secret key jamais entra aqui).
  'sb_publishable_QIHn7Eit-jesTppzvp_iiQ_qFfaFzNH';
...
async function checkMemberEntitlement(accessToken: string): Promise<MemberEntitlement> {
  ...
  try {
    response = await net.fetch(
      `${MEMBER_SUPABASE_URL}/rest/v1/enrollments?select=status,expires_at`,
      {
        headers: {
          apikey: MEMBER_SUPABASE_ANON_KEY,
...
void app.whenReady().then(async () => {
  ...
  await prepareCacheDirectories().catch((error: unknown) => {
    console.warn('Nao foi possivel preparar os caches do CenaRaiz:', error);
  });
  setupAutoUpdate();
  void memberBoot();
  ...
  void protocol.handle('cena-raiz-media', async (request) => {
