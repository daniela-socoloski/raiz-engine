...
import { CENA_RAIZ_INSTRUCTIONS } from './codex-app-server';
import type {
  ClaudeAccountState,
...
// manual: o site mostra o codigo e o aluno cola no CenaRaiz. O env existe para
// as sondas de teste nao disputarem a porta com um CenaRaiz aberto.
const OAUTH_CALLBACK_PORT = Number(process.env.CENA_RAIZ_OAUTH_CALLBACK_PORT) || 54545;
...
export type ClaudeAgentDeps = {
  ...
  // CLAUDE_CONFIG_DIR proprio do CenaRaiz: sessoes e estado isolados de
  ...
  authFile: string;
  // PATH das ferramentas empacotadas + CENA_RAIZ_* + caches, o mesmo do Codex.
...
export class ClaudeAgent {
    ...
    return new Promise((resolve) => {
      const server = createServer((request, response) => {
        ...
        if (!valid) {
          ...
          response.end(page('Requisição de login inválida', 'Volte ao CenaRaiz e tente de novo.'));
          return;
        }
        login.busy = true;
        ...
        response.end(
          page(
            'Quase lá',
            'Pode fechar esta aba e voltar para o CenaRaiz — o login está sendo concluído no aplicativo (pode levar até um minuto).',
          ),
        );
  ...
  // Fluxo manual: o site exibiu "codigo#estado" e o aluno colou no CenaRaiz.
  ...
  private buildEnvironment(auth: Record<string, string>): Record<string, string | undefined> {
    ...
    // pode vazar para o agente do CenaRaiz (ANTHROPIC_API_KEY teria precedencia
  ...
  async sendMessage(
    ...
    try {
      ...
      options: {
        ...
        // O prompt do Claude Code + o contrato do CenaRaiz, identico ao Codex.
        systemPrompt: {
          type: 'preset',
          preset: 'claude_code',
          append: `${CENA_RAIZ_INSTRUCTIONS}${CLAUDE_EXTRA_INSTRUCTIONS}`,
        },
        // Nada do ~/.claude do usuario entra aqui: sem settings, hooks,
        // CLAUDE.md ou MCPs da maquina — o CenaRaiz define tudo.
        settingSources: [],
