...
const qaProject: ProjectSummary = {
  directory: '/tmp/cena-raiz-interface-qa',
...
export function createQaBrowserApi(): CenaRaizDesktopApi {
  return {
    ...
    getCodexAccount: async () => (qaChatGptConnected
      ? {
          status: 'signed-in',
          account: { type: 'chatgpt', email: 'qa@cena-raiz.local', planType: 'qa' },
    ...
    getMemberAuth: async () => (
      new URLSearchParams(window.location.search).has('aluno')
        ? { status: 'signed-out' }
        : { status: 'signed-in', email: 'aluno@marcaraiz.com.br', name: 'Aluno QA' }
    ),
      ...
      if (!approvalPreviewScheduled && new URLSearchParams(window.location.search).has('approval')) {
        ...
        window.setTimeout(() => emit({
          ...
          approval: {
            ...
            cwd: '/Users/fillrocha/Documents/Coding/CenaRaiz/projeto de teste',
          },
        }), 300);
      }
