# Cena-raiz-desktop— contexto consolidado do projeto
...
correções e próximos passos definidos durante o desenvolvimento do cena-raiz. Ele existe para que uma nova sessão ou agente consiga continuar o
...
3. **Projetos individuais de vídeo**
   ...
   - Projeto usado para testar o Desktop: 
   - Arquivos de edição e renders pertencem ao projeto de vídeo, nunca ao
     repositório do Desktop ou da skill.

## 2. Visão do produto

O cena-raiz-desktop deve transformar a experiência da skill cena-raiz em um aplicativo
...
- Também recebe `CENA_RAIZ_PYTHON`, `CENA_RAIZ_FFMPEG`, `CENA_RAIZ_FFPROBE`, `CENA_RAIZ_UV`,
  `CENA_RAIZ_YTDLP`, `CENA_RAIZ_WHISPER_MODEL` e `CENA_RAIZ_HELPERS`.
...
- O modelo é fixo em `small` (`Systran/faster-whisper-small`, ~464 MB) e é
  informado ao agente por `CENA_RAIZ_WHISPER_MODEL`. Trocar o modelo exige mudar
...
## 5. Login e provedores de IA (ChatGPT + Claude + Gemini + higgsfield + magnific)

Desde a 0.10.0 o cena-raiz tem três provedores de IA; cada aluno conecta a própria
...
- O Codex usa um `CODEX_HOME` próprio dentro dos dados do cena-raiz.
...
- MODELO FIXO (0.13.6): o default do CLI 0.147.0 é `gpt-5.6-sol` (`isDefault`
  ...
  account" — visto na máquina de aluno). O cena-raiz crava `gpt-5.6-terra`
...
- O Desktop não deve depender da skill instalada no `CODEX_HOME` pessoal do
  ...
  `CENA_RAIZ_INSTRUCTIONS` exportada de `codex-app-server.ts`).
- O fuse de criptografia de cookies está desabilitado porque o cena-raiz não
  persiste cookies do Electron. Isso evitou o prompt desnecessário do macOS
  Keychain chamado “cena-raiz Safe Storage”.
...
- O design deve reutilizar o design system do cena-raiz; não criar uma identidade
  paralela.
...
- O preview exibe automaticamente a mídia mais recente. O protocolo
  `cena-raiz-media://` serve os arquivos com suporte a Range (206, sufixo,
...
- **Fontes locais embutidas (v2)**: o `@remotion/google-fonts` (63 MB) não
  ...
  **embutidos como data URIs** (primeira linha carrega a versão; mudou o
  ...
  `--log=verbose` e marcadores `cena-raiz-fonts` no console: o
...
- Os geradores oficiais estão embutidos em `resources/helpers/` (24 KB) e
  chegam ao agente pela variável `CENA_RAIZ_HELPERS`, sem cópia dentro do projeto:
...
- Fontes referenciadas são sondadas com FFprobe e recebem tokens
  `cena-raiz-media://` próprios; sem o arquivo, o limite de trim é o trecho já
  usado.
...
`/Users/fillrocha/Developer/cena-raiz-desktop/out/make/cena-raiz-0.8.2-arm64.dmg`

Configuração do DMG:

- Janela 660 × 400.
- cena-raiz em `(180, 220)`.
- Applications em `(480, 220)`.
- Fundo normal e Retina próprios.
- Ícone oficial do cena-raiz no volume.
- Layout centralizado e compacto.

O build local usa assinatura ad-hoc quando `CENA_RAIZ_MAC_SIGN_IDENTITY` não está
configurado.
...
- **Assinatura de produção**: `CENA_RAIZ_MAC_SIGN_IDENTITY="Developer ID
...
- **Notarização**: `CENA_RAIZ_APPLE_ID`, `CENA_RAIZ_APPLE_APP_PASSWORD` (senha de
  app de appleid.apple.com) e `CENA_RAIZ_APPLE_TEAM_ID`. Presentes as três +
...
- **OTA (Squirrel.Mac, o mesmo do app do ChatGPT)**: o aplicativo checa um
  ...
  (também aceita `CENA_RAIZ_UPDATE_FEED_URL` para teste).
...
("Preparando o cena-raiz"), e de novo apenas quando alguma versão do
...
- **Para ativar**: preencher `MEMBER_SUPABASE_URL` e
  `MEMBER_SUPABASE_ANON_KEY` em src/main.ts (ou env `CENA_RAIZ_SUPABASE_URL`/
  `CENA_RAIZ_SUPABASE_ANON_KEY` para teste) com a Project URL e a anon key do
...
- Opções do query espelham o modelo do Codex: `systemPrompt` preset
  `claude_code` + `CENA_RAIZ_INSTRUCTIONS`; `settingSources: []` (NADA do
...
- Ambiente: variáveis `ANTHROPIC_*`/`CLAUDE_*` herdadas são REMOVIDAS antes
  ...
  `CENA_RAIZ_*` + caches (mesmo env do Codex, `agentToolsEnvironment()` no
...
- Login OAuth: fluxo PKCE público do próprio Claude Code, nos endpoints
  ...
  retries curtos (2s/5s). CENA_RAIZ_OAUTH_CALLBACK_PORT muda a porta nas
...
- O agente de chat (qualquer provedor) escreve edit/imagens/pedidos.json
  ...
  CENA_RAIZ_INSTRUCTIONS. Depois de cada turno o renderer chama image:fulfill;
...
- Backend ChatGPT por CHAVE: generateOpenAiImage no main — POST
  ...
  app-server guarda no CODEX_HOME do cena-raiz (o app nunca teve copia
...
- Modelos padrao (nenhum fixado pelo cena-raiz, todos herdados dos motores):
  ...
  gemini-2.5-flash-image (pinado no cena-raiz).
...
- Sondagens que definiram o desenho (contra o CLI real, com chave falsa):
  ...
  `GEMINI_CLI_SYSTEM_SETTINGS_PATH` → arquivo do cena-raiz com
...
- Instruções: o ACP não tem prompt de sistema; as CENA_RAIZ_INSTRUCTIONS entram
...
artefato "cena-raiz-windows" (~1,1 GB) anexado na rodada. Falta instalar
...
- Python + WhisperX: stage na própria plataforma (por design); os ajustes
  ...
  avisam que no PowerShell a pasta de helpers é $env:CENA_RAIZ_HELPERS.
...
- Assinatura Windows (Azure Trusted Signing): o CI já está PRONTO e
  ...
  SDK, escreve o metadata.json e exporta CENA_RAIZ_WIN_SIGNTOOL +
  CENA_RAIZ_WIN_SIGN_PARAMS; o forge.config aplica windowsSign no packager
  (cena-raiz.exe) e no MakerSquirrel (Update.exe/Setup.exe via
  ...
  CENA_RAIZ_ATS_ENDPOINT (ex.: https://eus.codesigning.azure.net)/
  CENA_RAIZ_ATS_ACCOUNT/CENA_RAIZ_ATS_PROFILE no repositório. Primeira build
...
- (sem release) infra Windows completa, 2026-08-19: FFmpeg principal win32
  ...
  win (nupkg → RELEASES → Setup versionado + cena-raizSetup.exe estável);
...
- 0.13.5: login do Claude resiliente ao rate limit da Anthropic. Em uso
  ...
  cena-raiz") e a troca segue no aplicativo com novas tentativas (3s/8s/20s/
  ...
  turnos. Porta de callback ganhou env CENA_RAIZ_OAUTH_CALLBACK_PORT para as
  sondas não disputarem a 54545 com um cena-raiz aberto (lição: a sonda
...
- 0.11.1: continuação automática das imagens. Em uso real o ciclo não
  ...
  aplicação só viria se o aluno mandasse outra mensagem. Agora o cena-raiz
...
- 0.8.6: aba Estilos refinada — topo com um único título, sem numeração de
  ...
  ímpar). CENA_RAIZ_THUMBS_REUSE=1 reaproveita renders e itera só recortes.
...
- 0.7.7: refinamentos de UI/UX — mensagens disparadas pela interface mostram
  ...
  cena-raiz-media já autorizada); chip da faixa de legendas mostra só
...
- 0.7.4: o protocolo cena-raiz-media passou a servir Range (206/Accept-Ranges).
...
```bash
codesign --verify --deep --strict --verbose=2 out/cena-raiz-darwin-arm64/cena-raiz.app
plutil -extract CFBundleShortVersionString raw out/cena-raiz-darwin-arm64/cena-raiz.app/Contents/Info.plist
```

## 17. Convenções para continuar o desenvolvimento

- Trabalhar no repositório `cena-raiz-desktop`, salvo pedido explícito envolvendo a
