...
// (CENA_RAIZ_MSYS2_BASH aponta para outro bash se preciso). O mingw poe as DLLs
...
if (target === 'win32-x64') {
  ...
  const bash = process.env.CENA_RAIZ_MSYS2_BASH || 'C:\\msys64\\usr\\bin\\bash.exe';
  if (!(await exists(bash))) {
    throw new Error(
      `MSYS2 nao encontrado (${bash}). Instale o MSYS2 com mingw-w64-x86_64-toolchain e make, ou defina CENA_RAIZ_MSYS2_BASH.`,
    );
  }
