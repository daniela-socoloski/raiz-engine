...
const codexHome = await mkdtemp(path.join(tmpdir(), 'cena-raiz-codex-smoke-'));
...
try {
  const initialized = await request('initialize', {
    clientInfo: { name: 'cena_raiz_desktop_smoke', title: 'CenaRaiz smoke test', version: '0.1.0' },
  ...
  const thread = await request('thread/start', {
    ...
    serviceName: 'cena_raiz_desktop_smoke',
  });
