...
// 300 MiB e o ZIP do CenaRaiz passa de 800 MB. As credenciais S3 sao derivadas
...
//   CENA_RAIZ_CF_ACCOUNT_ID   conta Cloudflare
//   CENA_RAIZ_CF_API_TOKEN    token com permissao R2 Edit
//   CENA_RAIZ_R2_BUCKET       nome do bucket
//   CENA_RAIZ_UPDATE_BASE_URL URL publica do bucket (r2.dev ou dominio proprio)
...
const accountId = process.env.CENA_RAIZ_CF_ACCOUNT_ID?.trim();
const apiToken = process.env.CENA_RAIZ_CF_API_TOKEN?.trim();
const bucket = process.env.CENA_RAIZ_R2_BUCKET?.trim();
const baseUrl = process.env.CENA_RAIZ_UPDATE_BASE_URL?.trim()?.replace(/\/$/, '');

if (!accountId || !apiToken || !bucket || !baseUrl) {
  console.error('Preencha CENA_RAIZ_CF_ACCOUNT_ID, CENA_RAIZ_CF_API_TOKEN, CENA_RAIZ_R2_BUCKET e CENA_RAIZ_UPDATE_BASE_URL no signing.env.');
  process.exit(1);
}
...
const zipName = `CenaRaiz-darwin-arm64-${version}.zip`;
...
if (isWindowsRelease) {
  ...
  await putObject('CenaRaizSetup.exe', setupPath, 'application/octet-stream', setupSize);

  console.log(`\nRelease Windows ${version} publicado.`);
  console.log(`Atualizacao: ${baseUrl}/win32/RELEASES`);
  console.log(`Instalador: ${baseUrl}/CenaRaizSetup.exe`);
} else {
  const feed = {
  ...
  await putObject('CenaRaiz.dmg', dmgPath, 'application/x-apple-diskimage', dmgInfo.size);

  console.log(`\nRelease ${version} publicado.`);
  console.log(`Feed: ${baseUrl}/feed.json`);
  console.log(`Instalador: ${baseUrl}/CenaRaiz.dmg`);
}
