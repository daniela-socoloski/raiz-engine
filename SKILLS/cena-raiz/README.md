...
[daniela-socoloski/raiz-engine](https://github.com/daniela-socoloski/raiz-engine).

---

## Instalação

São **dois comandos**: um para os programas que a cena-raiz usa, outro para a cena-raiz.

### 1. Os programas (uma vez na vida)

- **`uv`** — gerenciador de pacotes Python. A cena-raiz usa para tudo, e ele também
...
- **`git`** — a cena-raiz não usa. É útil para o Claude Code e para quem for
...
- Não encosta numa instalação de desenvolvedor (pasta com `.git`) sem `--force`,
  e preserva suas configurações se você já tinha a cena-raiz instalada.
...
A cena-raiz usa [WhisperX](https://github.com/m-bain/whisperX): transcreve com o
...
procurar uma música pronta. Isso usa o Treblo, que é o único recurso da cena-raiz com
...
6. Clique em **Create Key**, escreva o nome **Cena Raiz** e confirme em **Create**.
7. Copie a chave.
