...
*/
/* cenaraiz preview — interactive editing timeline.
 ...
 * The one place that knows which looks cenaraiz can build. It is APP-level, not
...
let S = {
  ...
  jcutOpen: localStorage.getItem('cenaraiz.jcutOpen') === '1',
};
...
async function applyState(data) {
  ...
  $('projectName').textContent = S.state.project || 'cenaraiz';
...
$('jcutToggle').addEventListener('click', () => {
  ...
  localStorage.setItem('cenaraiz.jcutOpen', S.jcutOpen ? '1' : '0');
  renderJcutAudio();
  updateScrollRange();
  positionNeedle();
});
