...
REPO = "daniela-socoloski/raiz-engine"
SKILL_NAME = "cena-raiz"
...
def _validate_CENA_RAIZ_payload(src: Path) -> None:
...
def _copy_CENA_RAIZ_payload(src: Path, dest: Path) -> None:
    """Copy only the published skill payload from an cena-raiz checkout."""
...
def install_into(src: Path, skills_dir: Path, force: bool,
    ...
    if name == SKILL_NAME:
        ...
        _validate_CENA_RAIZ_payload(src)
    ...
    # dirs_exist_ok so the surviving .venv/.env keep their place. cena-raiz uses an
    # explicit allowlist; third-party skills (currently Remotion) retain their
    # own complete directory structure.
    if name == SKILL_NAME:
        _copy_CENA_RAIZ_payload(src, dest)
    else:
...
def _clone_is_pristine(repo: Path) -> bool:
    ...
    leftovers = [ln for ln in r.stdout.splitlines()
                 if ln.strip() and not ln[3:].startswith((".venv/", ".env", "cena-raiz.egg-info/"))]
...
def main() -> None:
    ...
    if not shutil.which("git"):
        log(f"  · git        — a cena-raiz não usa, mas o Claude Code sim. {hint('git')}")
    ...
    if ja_existia:
        ...
        log(f"Tudo pronto! A cena-raiz está instalada e pronta para usar no {onde}.")
        log()
        log("Reinicie o agente e abra uma nova sessão chamando a Skill cena-raiz.")
        log()
