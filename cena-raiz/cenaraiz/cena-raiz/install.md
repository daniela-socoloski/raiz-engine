---
name: cena-raiz-install
description: Verify or repair an cena-raiz install (Claude Code, Codex, or any agent with a skills directory). The user installs with one command from README.md; this file is for checking that it worked and fixing what didn't.
---
...
- **User layout** (what the installer produces, and what you should assume): the
  skill directory IS `~/.claude/skills/cena-raiz` or `~/.codex/skills/cena-raiz`.
...
- **Contributor layout**: repo cloned wherever the user keeps projects, plus a
  ...
  installer detects a `.git` there and refuses to overwrite it.
    ...
    ```bash
    # macOS / Linux
    ln -sfn ~/Developer/cena-raiz ~/.claude/skills/cena-raiz
    ln -sfn ~/Developer/cena-raiz ~/.codex/skills/cena-raiz
    ```

    ```powershell
    # Windows — a junction needs no admin rights, unlike a symlink
    New-Item -ItemType Junction -Path "$env:USERPROFILE\.claude\skills\cena-raiz" `
      -Target "$env:USERPROFILE\Developer\cena-raiz"
    New-Item -ItemType Junction -Path "$env:USERPROFILE\.codex\skills\cena-raiz" `
      -Target "$env:USERPROFILE\Developer\cena-raiz"
    ```
