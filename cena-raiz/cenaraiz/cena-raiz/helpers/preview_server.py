"""CenaRaiz preview server — serves the standard editing interface + session media.
...
def _check_access(root: Path) -> None:
    ...
    """
    probe = root / ".cena-raiz-write-probe"
...
def main() -> None:
    ap = argparse.ArgumentParser(description="CenaRaiz preview interface server")
    ...
    print(f"CenaRaiz preview → http://127.0.0.1:{args.port}  (root: {root})", flush=True)
    srv.serve_forever()
