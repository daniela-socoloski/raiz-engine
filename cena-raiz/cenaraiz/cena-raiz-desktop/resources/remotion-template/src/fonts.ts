...
// arquivos em fonts.gstatic.com durante o render, e o CenaRaiz renderiza sem
...
export function loadCenaRaizFonts(): void {
  if (loading) return;
  ...
  const handle = delayRender('Carregando as fontes locais do CenaRaiz', {
    timeoutInMilliseconds: 86_400_000,
  });
  // Os marcadores cena-raiz-fonts aparecem com --log=verbose e mostram onde o
  // carregamento parou quando um render morrer por timeout das fontes.
  console.log('cena-raiz-fonts: v2 iniciando');
  const load = (async () => {
    ...
    console.log('cena-raiz-fonts: folha carregada');
    ...
    console.log(`cena-raiz-fonts: ${document.fonts.size} faces prontas`);
  })();
  loading = Promise.race([
    ...
    new Promise<void>((resolve) => {
      setTimeout(() => {
        console.log('cena-raiz-fonts: backstop de tempo acionado');
        resolve();
      }, FONT_TIMEOUT_MS);
    }),
  ])
    ...
    .finally(() => {
      ...
      console.log('cena-raiz-fonts: handle liberado');
    });
}
